...
>
> Use Agent tool with `subagent_type: "general"`. Never run `aws` directly in your session.
...
- **Running `aws` directly**: Always delegate to subagent via Agent tool
...
3. If user didn't specify profile: infer from context or ask using AskUserQuestion tool
