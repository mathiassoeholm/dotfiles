...
> **CRITICAL:** You MUST use the Agent tool with `subagent_type: "general"` to execute Jira CLI commands. NEVER run `jira` commands directly in your bash session.

Example Agent invocation:

```
Agent(subagent_type="general", prompt="Use jira CLI to list current sprint issues: jira sprint list --current --plain")
```
