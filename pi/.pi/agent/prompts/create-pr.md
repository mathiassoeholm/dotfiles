---
description: Review the current git change set, confirm a short commit message, keep the current branch/worktree, then run `mbm integrate` with the right flags.
---

Create a pr for the current work using the MBM CLI only. Follow this exact workflow:
...
3. Use the combined picture of staged local changes and, on feature branches, the full `origin/main...HEAD` diff to produce:
   ...
   - a commit message of max 72 characters, imperative mood, no trailing period
...
7. Run `mbm --agent integrate "<approved-commit-message>" --yes --include-uncommitted --no-merge --body "<pr-body>"`.
   - Add `--return-to-branch` when the current directory is a worktree or the current branch is not `main`.
8. Never replace this workflow with manual `git commit`, `git push`, `gh pr create`, or any other git/GitHub fallback.
